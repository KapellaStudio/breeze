...
const privateRestartFilter=/filter\(\s*\(\s*\[,\s*t\s*\]\s*\)\s*=>\s*!\s*t\.private\s*\)/.test(main);
const privateRestartSink=/\btabs\s*:\s*persistable\b/.test(main);
ok('private tabs are omitted from restart state',privateRestartFilter&&privateRestartSink);
