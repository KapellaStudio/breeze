...
function bootstrapSource(endpointUrl, token, allowed){
  ...
  const lines = [
    ...
    '    if(!chrome.tabs.onActivated)chrome.tabs.onActivated=event();',
    ...
    '    if(!chrome.windows.onFocusChanged)chrome.windows.onFocusChanged=event();',
    '    if(chrome.windows.WINDOW_ID_NONE==null)chrome.windows.WINDOW_ID_NONE=-1;',
    "    for(const name of ['create','getAll','getCurrent','getLastFocused']){const method='windows.'+name;if(allowed.has(method)&&typeof chrome.windows[name]!=='function')chrome.windows[name]=(details,cb)=>{if(typeof details==='function'){cb=details;details={};}const clean=name==='create'?extDetails(details):(details&&typeof details==='object'?details:{});return callback(invoke(method,clean),cb);};}",
    ...
    "    if(browserApi&&browserApi.runtime&&browserApi.runtime.id){const mirror=(name,props)=>{try{const source=chrome[name];if(!source)return;if(!browserApi[name])browserApi[name]={};const target=browserApi[name];if(!target)return;for(const prop of props){if(source[prop]==null)continue;const value=source[prop];if(typeof value==='function'||target[prop]==null)target[prop]=value;}}catch{}};mirror('tabs',['create','query','get','getCurrent','update','remove','onRemoved','onUpdated','onActivated']);mirror('windows',['create','getAll','getCurrent','getLastFocused','update','remove','onRemoved','onFocusChanged','WINDOW_ID_NONE']);mirror('cookies',['get','getAll','set','remove']);mirror('notifications',['create','clear','onClicked']);mirror('commands',['getAll']);mirror('identity',['getRedirectURL','launchWebAuthFlow']);}",
    '  }catch{}',
    '})();',
    ''
  ];
