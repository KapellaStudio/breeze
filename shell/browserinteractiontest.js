...
app.whenReady().then(async () => {
  ...
  try {
    ...
    ok('daily-use interaction module loaded',
      await exec(`document.documentElement.dataset.usabilityPass==='1'`));
    ...
    ok('Brave is not the active Breeze search provider',
      searchDefault[0]?.provider === 'Google', searchDefault[0]?.provider || 'missing');
    ok('Brave is not offered in visible search settings',
      await exec(`![...document.querySelectorAll('#engRow .engBtn')].some(b=>/brave/i.test(b.textContent||''))`));
    ...
    ok('Continue reuses the already-open matching tab',
      afterContinue.length === countBeforeContinue && afterContinue.find(t=>t.active)?.id === continueId,
      `count=${afterContinue.length} active=${afterContinue.find(t=>t.active)?.id}`);

    /* Clean default chrome + weather -------------------------------------- */
    ok('browser starts without a forced side panel',
      await exec(`![...document.querySelectorAll('aside.panel')].some(p=>p.dataset.on==='1')`));
    ok('weather is present in the working toolbar',
      await exec(`!!document.querySelector('#toolbarWeather') && !!document.querySelector('#toolbarWeather').textContent.trim()`));
    ...
    await exec(`window.__BREEZE_SHELL__.listTabs().then(async tabs=>{const t=tabs.find(x=>x.active);if(t&&await window.__BREEZE_SHELL__.isBookmarked(t.id))await window.__BREEZE_SHELL__.toggleBookmark(t.id);})`);
    await wait(150);
    const bookmarkButtonShape = await exec(`(()=>{const b=document.querySelector('#toolbarBookmarkBtn');return !!b && !b.hasAttribute('data-panel') && b.title==='Bookmark this page';})()`);
    ok('bookmark toolbar is a one-click action, not a panel', bookmarkButtonShape);
    await exec(`document.querySelector('#toolbarBookmarkBtn').click()`);
    await wait(250);
    ...
    await exec(`document.querySelector('#toolbarBookmarkBtn').click()`);
    await wait(200);
    ok('second bookmark click removes it',
      await exec(`window.__BREEZE_SHELL__.listTabs().then(t=>{const a=t.find(x=>x.active);return window.__BREEZE_SHELL__.isBookmarked(a.id)})`) === false);

    /* Notes: no prompt, type and save ------------------------------------- */
    await exec(`document.querySelector('.tools [data-panel="notes"]').click()`);
    await wait(220);
    ok('Notes opens an inline composer instead of a prompt',
      await exec(`!!document.querySelector('aside[data-p="notes"][data-on="1"] #breezeQuickNote')`));
    const noteBody = 'Daily workflow regression note';
    await exec(`(()=>{const t=document.querySelector('#breezeQuickNote');t.value=${JSON.stringify(noteBody)};t.dispatchEvent(new Event('input',{bubbles:true}));document.querySelector('#breezeSaveNote').click();})()`);
    await wait(350);
    ...
    /* Remaining daily toolbar controls ----------------------------------- */
    for (const panel of ['queue','downloads','snapshots']) {
      await exec(`document.querySelector('.tools [data-panel="${panel}"]').click()`);
      await wait(100);
      ok(`${panel} toolbar button opens its real panel`,
        await exec(`document.querySelector('aside[data-p="${panel}"]').dataset.on==='1'`));
    }
    await exec(`closePanels()`);

    await exec(`document.querySelector('#flowBtn').click()`);
    await wait(100);
    ok('Breeze Flow button opens Flow', await exec(`document.documentElement.dataset.view==='flow'`));
    ...
    ok('Compact mode button changes real layout state', compactAfter !== compactBefore, `${compactBefore}->${compactAfter}`);
    await exec(`document.querySelector('#compactBtn').click()`);

    await exec(`document.querySelector('#extBtn').click()`);
    await wait(80);
    ok('Extensions button opens the extension popover',
      await exec(`document.querySelector('#extPop').dataset.on==='1'`));
    await exec(`closeAll()`);

    const themeBefore = await exec(`document.documentElement.dataset.theme`);
    await exec(`document.querySelector('.tools [data-theme-toggle]').click()`);
    const themeAfter = await exec(`document.documentElement.dataset.theme`);
    ok('Theme button changes Breeze chrome theme', themeAfter !== themeBefore, `${themeBefore}->${themeAfter}`);
    await exec(`document.querySelector('.tools [data-theme-toggle]').click()`);

    await exec(`document.querySelector('.tools [data-open="set"]').click()`);
    await wait(80);
    ok('three-dot button opens the application menu',
      await exec(`document.querySelector('#appMenu').dataset.on==='1'`));
    await exec(`closeAll()`);

    /* Split: two genuine native page views, then close it again. ---------- */
    const betaId = await exec(`window.__BREEZE_SHELL__.newTab({url:${JSON.stringify(site('beta'))}})`);
    await wait(850);
    await exec(`window.__BREEZE_SHELL__.selectTab(${Number(continueId)})`);
    await wait(220);
    await exec(`document.querySelector('#splitBtn').click()`);
    await wait(400);
    const splitOpen = await exec(`window.__BREEZE_SHELL__.splitState()`);
    ok('Split View button opens two real web panes',
      splitOpen?.active === true && splitOpen?.leftTabId != null && splitOpen?.rightTabId != null);
    await exec(`document.querySelector('#splitBtn').click()`);
    await wait(250);
    const splitClosed = await exec(`window.__BREEZE_SHELL__.splitState()`);
    ok('Split View button closes the split cleanly', splitClosed?.active === false);
    if (betaId) await exec(`window.__BREEZE_SHELL__.closeTab(${Number(betaId)})`);

    ok('window controls are connected to named shell actions',
      await exec(`(()=>{const s=window.__BREEZE_SHELL__;return ['minimize','toggleMaximize','close'].every(k=>typeof s[k]==='function') && ['winMin','winMax','winClose'].every(id=>!!document.getElementById(id));})()`));
