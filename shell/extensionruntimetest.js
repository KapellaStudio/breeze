...
(async()=>{
  ...
  write(source,'popup.js',`(async()=>{try{const current=await chrome.windows.getCurrent();const tabs=await chrome.tabs.query({active:true});const created=await chrome.tabs.create({url:'https://example.net/from-action-popup'});const commands=await chrome.commands.getAll();await chrome.tabs.remove(created.id);document.getElementById('page').textContent=JSON.stringify({ok:true,current,tabs,created,commands});}catch(err){document.getElementById('page').textContent=JSON.stringify({ok:false,error:String(err&&err.message||err)});}})();`);
