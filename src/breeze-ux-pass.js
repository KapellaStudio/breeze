...
(function(){
  ...
  window.addEventListener('keydown',async e=>{
    ...
    const done=()=>{root.dataset.navbusy='0';};
    if(active?.id!=null)S.navigate(active.id,value).catch(done);else S.newTab({url:value}).catch(done);
    input.value='';
  },true);
  ...
  async function refreshActive(){
    const tabs=await S.listTabs().catch(()=>[]);const active=(tabs||[]).find(t=>t.active);
    if(active?.id!=null)activeId=active.id;
    return activeId;
  }
  refreshActive();
  ...
  S.on('tab:loading',async ev=>{
    if(ev?.id!==activeId)await refreshActive();
    if(ev?.id===activeId)root.dataset.navbusy=ev.loading?'1':'0';
  });
