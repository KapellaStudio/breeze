...
runtime  = _read(SRC / 'breeze-runtime.js')
searchui = _read(SRC / 'breeze-search-ui.js')
omniui   = _read(SRC / 'breeze-omni.js')
splitui  = _read(SRC / 'breeze-split-ui.js')
extactions = _read(SRC / 'breeze-extension-actions.js')
firstgate = _read(SRC / 'breeze-first-run-gate.js')
...
for name in SHELLS:
    ...
    if name == 'breeze-desktop.html':
        if '</body>' not in html:
            ...
            modules = ''.join('<script>\n' + module + '\n</script>\n' for module in (launch, product, context, vaultui, homeui, runtime, searchui, omniui, splitui, extactions, firstgate, interactions))
